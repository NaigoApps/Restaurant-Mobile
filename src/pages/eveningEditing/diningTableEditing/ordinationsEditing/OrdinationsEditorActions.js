...
export default class OrdinationsEditorActions {
    ...
    static WIZARD = {
        ...
        SET_MODE: ActionsFactory.next(),
        EDIT_DISH: ActionsFactory.next(),
        SET_FREE_ADDITION: ActionsFactory.next(),
        ADD_ADDITION: ActionsFactory.next(),
        REMOVE_ADDITION: ActionsFactory.next(),
        SET_PRICE: ActionsFactory.next()
    };
    ...
    static showOptions(ordination) {
        dispatcher.fireEnd(this.SHOW_OPTIONS, ordination);
    }
    ...
    static setWizardMode(mode){
        dispatcher.fireEnd(OrdinationsEditorActions.WIZARD.SET_MODE, mode);
    }

    static editDish(dish){
        dispatcher.fireEnd(OrdinationsEditorActions.WIZARD.EDIT_DISH, dish);
    }

    static setFreeAddition(text){
        dispatcher.fireEnd(OrdinationsEditorActions.WIZARD.SET_FREE_ADDITION, text);
    }

    static addAddition(addition){
        dispatcher.fireEnd(OrdinationsEditorActions.WIZARD.ADD_ADDITION, addition);
    }

    static removeAddition(addition){
        dispatcher.fireEnd(OrdinationsEditorActions.WIZARD.REMOVE_ADDITION, addition);
    }

    static setPrice(price){
        dispatcher.fireEnd(OrdinationsEditorActions.WIZARD.SET_PRICE, price);
    }
    ...
    static selectWizardDish(ordination, wData) {
        wData.ordination = ordination;
        dispatcher.fireEnd(OrdinationsEditorActions.WIZARD.SELECT_DISH, wData);
    }
